...
class Parser(BaseParser):
    ...
    def parse_sample(self, data):
        ...
        B = time.time()
        print('get gsm id:' + str(B-A))

        # get SRX IDs corresponding to related GSM IDs using gsm_uid_concat and gsm_count
        A = time.time()
        print(gsm_uid_concat)
        print(len(gsm_uid_concat))
        import numpy as np
        tmp = np.array(gsm_uid_concat)
        np.savetxt('test_gsm_uid_concat.txt',tmp,fmt="%s")
        tmp2 = np.array(data)
        np.savetxt('test_data.txt',tmp2,fmt="%s")
        srx_data = get_esummary(EntrezConst.GEO, gsm_uid_concat)
        B = time.time()
        print('get esummary srx:' + str(B-A))
