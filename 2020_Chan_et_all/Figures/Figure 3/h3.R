...
colnames(df_bin_means) <- c("bin_name", "bin_mean_xRatio")

# 4.3) Assign the bin_means relevant for each record
df$bin_mean_xRatio <- mapply(function(bin_name) {
  df_bin_means[df_bin_means$bin_name == bin_name, c("bin_mean_xRatio")]
},
df[,c("bin_name")])
...
SalesVar = ggplot(Joined, aes(x = Label, y = Value, fill=Label)) + 
  ...
  ggtitle(wrapper("Low variance of gross sales positively influences audit yield", 
                  width=40)) +
